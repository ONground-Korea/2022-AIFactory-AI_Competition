...
from AIFactory.NDLinear.data import testDataset
...
for i in tqdm(range(10)):
    data_list = glob(f'C:/Users/hjs/PycharmProjects/pytorch/AIFactory/preprocess2/test/set_{i}/*.csv')
    ...
    for idx, data in enumerate(sorted(data_list)):
        ...
        if i == 9 and file_number == '7':
            save_df = pd.DataFrame([0 for i in range(28)])
        ...
        for (inputs, outputs) in test_loader:
            ...
            pred = pred.to('cpu').detach().squeeze(dim=2).numpy()
...
for k in range(10):
    ...
    for c in globals()[f'set_df_{k}'].columns:
        ...
        for i in range(14):
            ans_1_14.append(
                (globals()[f'set_df_{k}'][c].iloc[i + 1] - base_d) / base_d)  # t+1 ~ t+14 까지는 (t+n - t)/t 로 계산

        ans_22_28 = (globals()[f'set_df_{k}'][c][
                     22:29].mean() - base_d) / base_d  # t+22 ~ t+28은 np.mean(t+22 ~ t+28) - t / t
...
# 최종 저장
submit_df.to_csv(f'submit_{pat}.csv', index=False, encoding='utf-8-sig')
